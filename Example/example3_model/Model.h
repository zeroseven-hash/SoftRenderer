...
public:
    ...
    void Draw(Renderer& renderer, Shader<V>& shader)
    {
        for (uint32_t i = 0;i < m_textures.size();i++)
        {
            std::string& name = m_textures[i].type_;
            if (name == "texture_diffuse") shader.SetTexture(0, (Canvas*)&m_textures[i]);
            m_textures[i].SaveFileBMP("texture.bmp");
        }
        renderer.DrawArray(m_vao, shader);
    }
private:
    VertexArrayBuffer<V, uint32_t> m_vao;
...
Mesh<V> Model<V>::ProcessMesh(aiMesh* mesh, const aiScene* scene)
{
    ...
    for (size_t i = 0;i < mesh->mNumVertices;i++)
    {
        ...
        {
            vertex.tex_coords_.x_ = mesh->mTextureCoords[0][i].x;
            vertex.tex_coords_.y_ = mesh->mTextureCoords[0][i].y;
        }
        else
            vertex.tex_coords_ = TinyMath::Vec2f(0.0f, 0.0f);
